...
i := DBGridUsers.SelectedIndex;
 ...
 if i = 1 then
   begin
     ...
     if not InputQuery('Ввод данных','Введите Логин', strUpdate)
      then exit;
 ...
 if i = 2 then
   begin
     strUpdate := DBGridUsers.DataSource.DataSet.Fields[i].Value;
     idUser := DBGridUsers.DataSource.DataSet.Fields[0].Value;
     if not InputQuery('Ввод данных','Введите имя', strUpdate)
      then exit;
       MySQLQueryDBGrid.SQL.Text := 'UPDATE `ListUser` SET '+
...
begin
     ...
     DBGridUsers.Columns[1].Title.caption:='Логин пользователя';
     DBGridUsers.Columns[2].Title.caption:='Имя пользователя';
...
begin
    ...
    if not InputQuery('Ввод данных','Введите название категории', NameCategory)
    then exit;
...
begin
   ...
   str := 'Вы действительно хотите удалить категорию "'+CategoryName+'"?';
   case MessageBox(Wnd,Pchar(str),'Окно предупреждения',Tip) of
     IDYES:
    begin
     CategoryID := DBLookupListBoxCategory.KeyValue;
...
begin
   ...
   str := 'Вы действительно хотите удалить пользователя "'+User+'"?';
   case MessageBox(Wnd,Pchar(str),'Окно предупреждения',Tip) of
     IDYES:
     begin
